...
def scrape(id_list: List[int], quiet: bool = False) -> List[OrderedDict[int, Any]]:
    ...
    for id in id_list:
        property_id = str(id)
        property_site = requests.get(f"{starting_url}{property_id}", timeout=(3, 27))
        ...
        property_detail = OrderedDict(
            {section.get("id"): section for section in property_soup.find_all("div", class_="details")}
        )
        # Extract the basic details into a dictionary
        basic_details = [
            detail
            for detail in property_detail["propertyDetails"].find_all("td")
            if "detailTitle" not in detail.get("class", []) and bool(str(detail.string).strip())
        ]
        stripped_details = []
        ...
        for i, detail in enumerate(values_details):
            if detail is not None and detail.endswith(":"):
                values_details_dict.update(
                    {convert_key(detail.split(" ", 1)[1].replace(":", "")): currency_str_convert(values_details[i + 2])}
                )
        property_info_structured.update(values_details_dict)
        # Extract the improvement details
        all_improvements = []
        improvement_headers = [
            detail for detail in property_detail["improvementBuildingDetails"].find_all("table", class_="improvements")
        ]
        improvement_details = [
            detail
            for detail in property_detail["improvementBuildingDetails"].find_all("table", class_="improvementDetails")
        ]
        total_living_area = 0
        num_improvements = 0
        for headers, details in zip(improvement_headers, improvement_details):
            num_improvements += 1
            improvements_dict = OrderedDict(
                {
                    re.sub(r"\#\d", "type", convert_key(key.string)): str(detail.string).strip()
                    for key, detail in zip(headers.find_all("th"), headers.find_all("td"))
                }
            )
            ...
            improvement_detail_rows = [
                detail.find_all("td", class_="")
                for detail in details.find_all("tr")
                if detail.find_all("td", class_="")
            ]
            for row in improvement_detail_rows:
                row = [detail.string for detail in row]
                improvement_details_dict = OrderedDict(
                    {convert_key(key): value for key, value in zip(improvement_detail_headers, row)}
                )
        ...
        land_details = OrderedDict(
            {
                f"land_{convert_key(key.string)}": value.string.strip()
                for key, value in zip(
                    property_detail["landDetails"].find_all("th"), property_detail["landDetails"].find_all("td")
                )
            }
        )
        if land_details:
        ...
        history_rows = [
            record.find_all("td") for record in property_detail["rollHistoryDetails"].find_all("tr", class_="")
        ]
        for row in history_rows:
            row = [value.string for value in row]
            row_dict = OrderedDict(
                {convert_key(key): currency_str_convert(value.string) for key, value in zip(history_headers, row)}
            )
