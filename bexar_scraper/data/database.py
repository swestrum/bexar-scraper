...
def insert_record(data_dict: OrderedDict[str, Any]):
    ...
    cursor.execute(f"INSERT INTO property VALUES({'?, ' * (len(data_dict.keys()) - 1) + '?'})", list(data_dict.values()))
    for improvement in improvement_list:
        ...
        for improvement_detail in improvement_details_list:
            cursor.execute(f"INSERT INTO improvement_details VALUES({'?, ' * (len(improvement_detail.keys()) - 1) + '?'})", list(improvement_detail.values()))
        cursor.execute(f"INSERT INTO improvement VALUES({'?, ' * (len(improvement.keys()) - 1) + '?'})", list(improvement.values()))
    for history in value_history_list:
        cursor.execute(f"INSERT INTO value_history VALUES({'?, ' * (len(history.keys()) - 1) + '?'})", list(history.values()))
    connection.commit()
